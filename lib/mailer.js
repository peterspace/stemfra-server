...
async function sendViaResend({ from, to, replyTo, subject, text, html, attachments, headers, messageId }) {
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      // RFC threading headers (In-Reply-To/References) + our own Message-ID so
      // multiple replies to the same lead thread in the recipient's client.
      headers: (headers || messageId)
        ? { ...(headers || {}), ...(messageId ? { 'Message-ID': messageId } : {}) }
        : undefined,
    }),
  });
...
async function sendMail({ fromName, to, replyTo, subject, text, html, attachments, headers, messageId }) {
  if (!to) return false;
  const provider = activeProvider();
  if (!provider) return false;
  const from = fromHeader(fromName, provider);
  // Truthy object keeps every existing `if (!sent)` caller working; `id` is
  // the RFC Message-ID when the caller supplied one (or nodemailer's).
  if (provider === 'resend') {
    await sendViaResend({ from, to, replyTo, subject, text, html, attachments, headers, messageId });
    return { ok: true, id: messageId || null };
  }
  const info = await gmailTransporter().sendMail({
    from, replyTo: replyTo || undefined, to, subject, text, html, attachments,
    headers: headers || undefined, messageId: messageId || undefined,
  });
  return { ok: true, id: info?.messageId || messageId || null };
}
