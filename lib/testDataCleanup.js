...
const { TEST_EMAIL_DOMAINS, TEST_EMAILS, isTestSite } = require('./testData');
...
async function plan() {
  ...
  const emailFilter = [...TEST_EMAIL_DOMAINS.map((d) => `email.ilike.%@${d}`), ...[...TEST_EMAILS].map((e) => `email.eq.${e}`)].join(',');
