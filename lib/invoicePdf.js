...
const methodLabel = (provider) =>
  provider === 'payoneer' ? 'Payoneer' : provider === 'stripe' ? 'Card on file'
    : provider === 'pending' || provider === 'airwallex' ? 'Bank transfer' : (provider || '—');
...
function commissionItems(charge, cur) {
  ...
  const streams = [
    ...
    ['memberships', li.membership_runrate_cents],
...
function drawInvoice(doc, { charge, contact, billingProfile = {}, provider, bank }) {
  ...
  // Commission + domain-adjustment invoices are paid by bank transfer to our Airwallex
  // Global Account (bank), referencing the invoice number; the tenant then uploads the
  // receipt in the CMS. Rendered as its own shaded panel for contrast. Subscription
  // charges keep the provider-specific one-liner.
  const bankTransfer = !paid && bank && (charge.kind === 'commission' || charge.kind === 'adjustment');
  if (paid) {
    ...
    doc.font('Helvetica').fontSize(9).fillColor(muted).text('Payment is collected via the Payoneer payment request sent to your account email.', 50, ty, { width: 495 });
  } else {
    doc.font('Helvetica').fontSize(9).fillColor(muted).text('Payment will be collected via your saved payment method on the due date.', 50, ty, { width: 495 });
  }
