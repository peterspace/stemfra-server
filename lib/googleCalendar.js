...
/**
 * Create a calendar event WITH a Google Meet link and email the attendees the
 * invite. Returns { eventId, meetLink, htmlLink }.
 */
async function createEventWithMeet({ summary, description, startIso, endIso, timeZone, attendees = [], calendarId = 'primary' }) {
  const event = {
    summary,
    description,
...
/** Cancel an event (notifies attendees). Best-effort — callers can ignore failures. */
async function deleteEvent({ eventId, calendarId = 'primary' }) {
  await calFetch(`/calendars/${encodeURIComponent(calendarId)}/events/${encodeURIComponent(eventId)}`, {
    method: 'DELETE', subject: hostEmail(), query: { sendUpdates: 'all' },
  });
}
