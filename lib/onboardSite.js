...
const { verifyClaimToken } = require('./claimTokens');
...
async function onboardCustomer({
  ...
  if (claimToken) {
    try {
      const leadId = verifyClaimToken(claimToken);
      if (leadId) {
        await supabase.from('leads').update({ stage: 'won', outreach_status: 'converted', engagement_status: 'converted', contact_id: contactId, last_activity_at: new Date().toISOString() }).eq('id', leadId);
