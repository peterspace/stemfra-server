...
async function onboardCustomer({
  name, email, password, company, vertical, city = null, templateSlug = null,
  // KYC/KYB + the two onboarding questions (all optional, additive).
  firstName = null, lastName = null, country = null, state = null,
  bookingProvider = null, bookingUrl = null, paymentMethods = null, tier = null,
}) {
  ...
  try {
    ...
    const patch = {};
    if (bookingProvider && bookingProvider !== 'stemfra') {
      if (bookingProvider === 'none') { patch.booking_mode = 'consultation_form'; patch.booking_config = {}; }
      else if (bookingUrl) { patch.booking_mode = 'link_out'; patch.booking_config = { provider: bookingProvider, booking_url: bookingUrl }; }
    }
    if (tier || paymentMethods || bookingProvider) {
      const { data: cur } = await supabase.from('sites').select('metadata').eq('id', site.siteId).single();
      patch.metadata = { ...(cur?.metadata || {}), onboarding: { tier: tier || null, payment_methods: paymentMethods || null, booking_provider: bookingProvider || 'stemfra' } };
    }
