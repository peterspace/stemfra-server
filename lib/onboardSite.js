  ...
  if (claimToken) {
    try {
      ...
      if (leadId) {
        ...
        const { error: leadErr } = await supabase.from('leads').update({ stage: 'won', outreach_status: 'converted', engagement_status: 'closed', contact_id: contactId, last_activity_at: new Date().toISOString() }).eq('id', leadId);
