...
async function buildOnboardingChecklist(siteId) {
  ...
  const steps = [
    ...
    { key: 'hero', kind: 'personalize', label: 'Personalize your homepage headline', hint: 'Make the first thing visitors see your own. I can draft it for you.', route: CMS_ROUTES.homeHero, draftable: true, auto: gate.hero_headline === true && gate.personalize_hero === undefined }, // personalize_hero only appears while it still equals the demo's
    ...
    // Contact details live in the home page's Location section (address/phone/
    // email feed the contact page + footer from there) — deep-link opens it.
    { key: 'contact', kind: 'personalize', label: 'Add your contact details', hint: 'Your own location name, address, phone and email (not the demo\'s) so customers can reach you.', route: CMS_ROUTES.homeContact, auto: ['location_name', 'address', 'phone', 'email'].every((k) => gate[k] === true) },
