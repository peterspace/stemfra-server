...
// vertical slug → the ONE Pages project that serves every site of that vertical
const VERTICAL_PROJECT = {
  barbershops:  'stemfra-barbers',
  salons:       'stemfra-salons',
  crossfit:     'stemfra-crossfit',
  yoga_pilates: 'stemfra-yoga',
};
const ZONE_SUFFIX = 'stemfra.com';
const sleep = (ms) => new Promise((r) => setTimeout(r, ms));

function projectFor(verticalSlug) {
  const p = VERTICAL_PROJECT[verticalSlug];
  if (!p) throw new Error(`no Pages project mapped for vertical "${verticalSlug}"`);
  return p;
}
