...
async function attachSiteDomain(siteId, { dryRun = false, pollMs = 3000, timeoutMs = 15000 } = {}) {
  ...
  // slot consumed. Only a BYO brand domain still needs the Pages custom-domain
  // attach (best-effort; its cert stays pending until the customer points DNS).
  if (WILDCARD) {
    let customDomainStatus = null;
    if (site.custom_domain && project && (await cf.getProject(project))) {
