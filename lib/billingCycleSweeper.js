...
async function dunOverdue() {
  ...
  const { data: overdue } = await supabase
    .from('billing_charges')
    .select('id, due_date, status, metadata, site:sites(id, metadata)')
  ...
  for (const c of overdue) {
    // Demo/test sites are never dunned (lib/testData.js): their charges are
    // seeds, rehearsals or Peter's own tests, and the "owner" is our inbox.
