...
async function sweepOnce() {
  ...
  for (const lead of leads) {
    ...
    // Auto speed-to-lead: a real reply → call them now, gated by the guardrails
    // (DNC + safe calling window + daily cap) and callability.
    const guard = (status === 'replied' && autoCall) ? await canAutoCall(lead) : { ok: false };
    if (guard.ok && leadgenCall.isConfigured() && leadgenCall.toE164(lead.phone, lead.phone_country)) {
      try {
