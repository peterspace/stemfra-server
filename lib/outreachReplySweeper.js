...
// Resolve sender (rep) mailbox addresses from profiles, in one query.
async function repEmailMap(ids) {
  const uniq = [...new Set(ids.filter(Boolean))];
  if (!uniq.length) return {};
  const { data } = await supabase.from('profiles').select('id, email').in('id', uniq);
  const map = {};
...
async function sweepOnce() {
  ...
  const reps = await repEmailMap(leads.map(l => l.outreach_sent_by));
  const autoCall = await autoCallEnabled();
  let flipped = 0;
  for (const lead of leads) {
    const repEmail = reps[lead.outreach_sent_by];
    if (!repEmail) continue;
    let reply;
    try {
      reply = await gmail.checkThreadForReply({ repEmail, threadId: lead.outreach_thread_id });
    } catch {
