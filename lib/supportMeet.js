...
/**
 * Fire-and-forget after a support-site booking commits: create the Calendar
 * event with Meet, invite the customer, and stamp the event id + Meet link on
 * the booking's metadata (so a later cancel can delete the event). Never
 * throws — the booking stands even if Google is down.
 */
async function createSupportCallMeet({ site, booking, service, customer }) {
  if (!isSupportSite(site) || !gc.isConfigured()) return;
  try {
