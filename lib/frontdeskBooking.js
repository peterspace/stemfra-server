...
const detailsCard = (title) => ({
  ...
  hint: 'We need an email or phone so we can send your confirmation.',
...
//   'auto'   (default) — offered only when the site has no card rail, so a studio
//                        that takes money at the door completes the booking in chat
//                        instead of dead-ending the visitor on /book
...
const payAtVenueAllowed = (site) => {
  ...
  return site?.payments_enabled !== true;
};
...
async function runBookingTool({ site, booking, zone, userMessage }) {
  ...
  if (timeOk) {
    return {
      note: `BOOKING NOTE: ${serviceName} on ${fmtDate(date, zone)} at ${fmtTime(time)} is held. A short form asking for their name and email or phone is shown. In ONE short line ask them to fill it in. Do NOT ask for the details in your text and do NOT offer times again.`,
  ...
  return {
    note: `AVAILABLE TIMES for ${serviceName} ${barberClause} on ${fmtDate(date, zone)}: ${fmtSlots(slots)}. Offer these real times only (a calendar showing them is displayed). Once the visitor picks a time AND you have their name plus an email or phone, the system will show a confirmation card. Set booking.confirm=true ONLY after they confirm.`,
    card: calendarCard({
...
async function classBookingFlow({ site, booking, service, serviceName, userMessage }) {
  ...
  if (!chosen) {
    ...
    return {
      ...
      note: `A CALENDAR of real ${serviceName} classes is displayed beside the chat: ${sessions.length} session${sessions.length === 1 ? '' : 's'} across ${dates.length} day${dates.length === 1 ? '' : 's'}, the first on ${fmtDate(dates[0], zone)}. In ONE short line, invite the visitor to pick a day and time from the calendar. Do NOT list the classes, dates or times in your text — the calendar already shows them. Once they pick one AND you have their name plus an email or phone, the system shows a confirmation card. Set booking.confirm=true ONLY after they confirm.`,
      card: calendarCard({
  ...
  if (!ready) {
    return {
      note: `BOOKING NOTE: ${serviceName} on ${chosen.label}${chosen.instructor ? ` with ${chosen.instructor}` : ''} is held. A short form asking for their name and email or phone is shown. In ONE short line ask them to fill it in. Do NOT ask for the details in your text. Don't set confirm yet.`,
      card: detailsCard('Your details'),
    };
  }
