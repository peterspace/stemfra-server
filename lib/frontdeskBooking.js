...
async function runBookingTool({ site, booking, zone }) {
  ...
  // GATE: only Stemfra's native engine can book in chat. If the site uses an
  // external booking system, hand off its link (tappable) and never collect
  // times for a system we don't run. "No online booking" → a contact handoff.
  const mode = site.booking_mode || 'native';
  if (mode !== 'native') {
    const cfg = site.booking_config || {};
    if (mode === 'link_out' && cfg.booking_url) {
      const via = cfg.provider_name ? `via ${cfg.provider_name}` : 'online';
      return {
        note: `BOOKING NOTE: ${bizName} takes bookings on an external system (${cfg.provider_name || 'their booking page'}). Do NOT ask for a service/date/time and do NOT say you booked anything. In ONE short line, invite the visitor to tap the button to book ${via}.`,
        card: { kind: 'handoff_booking', title: `Book ${via}`, lines: [`Book with ${bizName}`], actions: [{ label: 'Book now', href: cfg.booking_url }] },
      };
    }
    return {
