...
async function classBookingFlow({ site, booking, service, serviceName, userMessage }) {
  ...
  if (!chosen) {
    ...
    return {
      note: `UPCOMING ${serviceName.toUpperCase()} CLASSES (offer ONLY these — a calendar with these classes is shown): ${lines}. In ONE short line, invite the visitor to pick a day and time from the calendar. Once they pick one AND you have their name plus an email or phone, the system shows a confirmation card. Set booking.confirm=true ONLY after they confirm.`,
      card: calendarCard({
...
async function confirmStep({ site, service, serviceName, lines, title, pending, userMessage, isClass }) {
  ...
  if (isClass) {
    ...
    if (packs.length) {
      ...
      // A pass was chosen → show it on the receipt and send them to checkout.
      // (We don't charge inline here: createBookingIntent prices from the
      // SERVICE, so charging a pass price would bill the wrong amount.)
      const packClasses = chosenPack.metadata?.classes;
      return {
        ...
        card: {
          ...
          title: 'Buy and book',
          lines: [...lines, en(chosenPack.name), packClasses ? `${packClasses} class${Number(packClasses) === 1 ? '' : 'es'}` : null].filter(Boolean),
          price: `$${(chosenPack.price_cents / 100).toFixed(0)}`,
          actions: [
