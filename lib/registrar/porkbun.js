...
// All domains on our Porkbun account (the CRM Domains monitor).
async function listDomains() {
  const data = await pbPost('/domain/listAll', { includeLabels: 'no' });
  return (data.domains || []).map(d => ({
    ...
    whoisPrivacy: d.whoisPrivacy === '1' || d.whoisPrivacy === 1,
  }));
}
