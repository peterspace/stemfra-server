...
async function attachCustomDomain(projectName, fqdn) {
  ...
  try {
    ...
    // 8000049 "already exists" / 8000018 "You have already added this custom
    // domain" — both mean the attach is a no-op.
    if (/already (exists|in use|added)/i.test(JSON.stringify(err.response?.data?.errors || ''))) {
