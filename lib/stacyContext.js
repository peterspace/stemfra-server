...
const { CMS_GUIDE } = require('./cmsRoutes');

const en = (v) => (v && typeof v === 'object' ? (v.en ?? '') : (v || ''));

/**
 * @param {string} siteId
 * @param {{ includeCmsMap?: boolean }} [opts] — `includeCmsMap` adds `cms_map`
 *   (owner-task → where in the CMS + deep link, from lib/cmsRoutes.js CMS_GUIDE)
 *   so Stacy can answer "where do I change X?" from real routes. Stacy-ONLY:
 *   the public Front Desk chat (visitors) must never receive CMS navigation.
 */
async function buildSiteContext(siteId, { includeCmsMap = false } = {}) {
  ...
  return {
    ...
    // Where things live in the CMS (Stacy only). `where` = the sidebar path the
    // owner sees; `route` = the deep link (the CMS opens it from Stacy's guidance).
    ...(includeCmsMap ? { cms_map: CMS_GUIDE } : {}),
  };
}
