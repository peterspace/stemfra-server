...
// airwallex = the live collection method (bank transfer, commission model);
// payoneer stays registered but dormant (COMMISSION_MODEL.md §2).
const PROVIDERS = { airwallex, payoneer };

async function getActiveProvider() {
  try {
    const { data } = await supabase.from('crm_settings').select('value').eq('key', 'billing_active_provider').maybeSingle();
    return data?.value?.provider || 'airwallex';
  } catch { return 'airwallex'; }
}
...
function providerFor(key) { return PROVIDERS[key] || airwallex; }
...
async function markRequested(chargeId, { externalRef = null, by = null } = {}) {
  ...
  if (error) throw new Error(error.message);
  // $0 charge (e.g. a no-sales commission month): mark it requested for ledger
  // consistency, but skip the mirror + email — there is nothing to collect, and
  // a $0 Airwallex invoice auto-settles into an unremovable "paid" artifact
  // (Peter's call, 2026-08-12).
  if ((data.amount_cents || 0) <= 0) return data;
  // Mirror the invoice into Airwallex Billing FIRST (awaited, best-effort). This
  // creates the official OUT_OF_BAND invoice, builds KYB trading history, and
  // stores metadata.awx_invoice_id — which the payment-request email then reads
  // to carry the canonical hosted "View invoice online" link + the Airwallex PDF.
  // mirrorInvoice swallows its own errors and returns null, so this never throws;
  // if it fails, the email falls back to our own rendered PDF. See
  // lib/airwallexBilling.js + lib/billingEmails.js airwallexInvoiceAssets().
  try { await require('../airwallexBilling').mirrorInvoice(chargeId); } catch (e) { console.error('[billing] mirror failed:', e.message); }
  // Then email the business owner the invoice / payment request (best-effort).
...
async function markPaid(chargeId, { by = null } = {}) {
  ...
  // Settle the mirrored Airwallex invoice (best-effort).
  try { require('../airwallexBilling').markInvoicePaid(chargeId).catch(() => {}); } catch { /* mirror optional */ }
  // Pay-and-publish: an INITIAL charge paid opens the billing gate + auto-publishes
  // the site (if its checklist passes). Fire-and-forget — never blocks payment.
  // Lazy-require avoids a load cycle (publishOnPayment → sitePublish → …).
  try { require('./publishOnPayment').onInitialChargePaid(data).catch(() => {}); } catch { /* optional */ }
  return data;
}
