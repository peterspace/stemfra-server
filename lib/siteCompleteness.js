...
async function evaluateCompleteness(siteId) {
  ...
  const required = [
    ...
    { key: 'address', label: 'Address', ok: nonEmpty(loc.address), hint: 'Add your street address to the Location section.', route: CMS_ROUTES.homeContact },
    { key: 'phone', label: 'Phone number', ok: nonEmpty(loc.phone), hint: 'Add a contact phone to the Location section.', route: CMS_ROUTES.homeContact },
  ...
  try {
    ...
    if (seedId && seedId !== siteId) {
      ...
      if (nonEmpty(loc.address) && loc.address === seedLoc.address) {
        recommended.push({ key: 'personalize_address', label: 'Update the address', ok: false, hint: 'Still showing the demo address.', route: CMS_ROUTES.homeContact });
      }
    }
