...
async function evaluateCompleteness(siteId) {
  ...
  const required = [
    ...
    { key: 'address', label: 'Address', ok: nonEmpty(loc.address), hint: 'Add your street address to the Location section.', route: CMS_ROUTES.homeContent },
    { key: 'phone', label: 'Phone number', ok: nonEmpty(loc.phone), hint: 'Add a contact phone to the Location section.', route: CMS_ROUTES.homeContent },
    ...
    { key: 'hero_headline', label: 'Homepage headline', ok: nonEmpty(i18nEn(hero.headline_i18n)), hint: 'Set your homepage hero headline.', route: CMS_ROUTES.homeContent },
  ];
  ...
  try {
    ...
    if (seedId && seedId !== siteId) {
      ...
      if (heroHeadline && heroHeadline === i18nEn(seedHero.headline_i18n)) {
        recommended.push({ key: 'personalize_hero', label: 'Personalize the homepage headline', ok: false, hint: 'Still showing the demo headline — make it yours.', route: CMS_ROUTES.homeContent });
      }
      if (nonEmpty(loc.address) && loc.address === seedLoc.address) {
        recommended.push({ key: 'personalize_address', label: 'Update the address', ok: false, hint: 'Still showing the demo address.', route: CMS_ROUTES.homeContent });
      }
    }
