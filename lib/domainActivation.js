...
const MIN_WAIT_MS = Number(process.env.DOMAIN_ACTIVATION_MIN_WAIT_MS || 10 * 60 * 1000); // 10 min floor (Cloudflare is usually live in <15; the probe is the real gate — Peter, 2026-08-20)
...
async function sweepOnce() {
  ...
  for (const s of sites || []) {
    ...
    if (age < MIN_WAIT_MS) continue;                  // let DNS settle first
    ...
    await supabase.from('cms_notifications').insert([{
      site_id: s.id, type: 'domain_active', category: 'website', // category CHECK: operations|billing|website|product|security
