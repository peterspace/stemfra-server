  ...
  try {
    ...
    try {
      if (c.notify.owner_cancellation && !canceledByBusiness) {
        sendOwnerSms(c.ownerAuthUserId, `${c.businessName}: booking canceled. ${c.customerName || 'A customer'}, ${c.serviceName}, ${c.dateLabel} at ${c.timeLabel}.`);
...
async function sendRescheduleEmails(bookingId, { oldStartsAtISO = null } = {}) {
  ...
  try {
    ...
    if (c.notify.owner_reschedule) {
      try {
        sendOwnerSms(c.ownerAuthUserId, `${c.businessName}: booking moved. ${c.customerName || 'A customer'}, ${c.serviceName}, now ${c.dateLabel} at ${c.timeLabel}.`);
        const dashboardUrl = await cmsMagicLink(c.ownerAuthUserId, '/bookings');
        out.owner = await send({
          fromName: 'STEMfra Sites', to: c.ownerEmail,
...
async function sendOwnerNewBookingEmail(bookingId) {
  try {
    ...
    sendOwnerSms(c.ownerAuthUserId, `${c.businessName}: new booking. ${c.customerName || 'A customer'}, ${c.serviceName}, ${c.dateLabel} at ${c.timeLabel}.`);
    const dashboardUrl = await cmsMagicLink(c.ownerAuthUserId, '/bookings');
    return await send({
      fromName: 'STEMfra Sites', to: c.ownerEmail,
