...
function previewUrlFor(site) {
  // Custom domain only once it is ACTIVE (lib/domainActivation.js); until then
  // the stemfra.com address is the public one.
  return `https://${require('./domainActivation').publicHostFor(site, ZONE)}`;
}
...
async function listStarters({ vertical = null } = {}) {
  ...
  return (data || [])
    ...
    .map((s) => ({
      ...
      // The public URL to open (custom domain when the demo has one, else the
      // stemfra.com host). The client MUST NOT derive the subdomain from this
      // (a custom domain has no `.stemfra.com` to strip) — use `subdomain`.
      previewUrl: previewUrlFor(s),
      // Stable key for the demo — mockup/screenshot maps and `?site=` params.
