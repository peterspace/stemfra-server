  ...
  const doc = new Document({
    sections: [{
      children: [
        ...
        ...(m.membershipCollectedCents > 0 ? [line(`Membership payments collected this period: ${money(m.membershipCollectedCents)} from ${m.membershipCollectedCount} confirmation${m.membershipCollectedCount === 1 ? '' : 's'} (collected in person, separate from the one-time revenue above).`)] : []),
        ...(m.membershipDueCents > 0 ? [line(`${money(m.membershipDueCents)} due from ${m.membershipDueCount} member${m.membershipDueCount === 1 ? '' : 's'} whose renewal is outstanding — confirm collections in Memberships → Renewals.`)] : []),
        ...(m.membershipMrrCents > 0 ? [line(`Active memberships run-rate: ${money(m.membershipMrrCents)}/mo from ${m.membershipCount} active member${m.membershipCount === 1 ? '' : 's'} (a forward estimate, not collected revenue).`)] : []),
