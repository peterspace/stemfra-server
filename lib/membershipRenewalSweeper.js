...
const { getSiteNotifyPrefs } = require('./notifyPrefs');
...
async function sweepMembershipRenewals({ dryRun = false, includeDemo = false } = {}) {
  ...
  for (const sub of subs || []) {
    if (!includeDemo && isNonProductionSite(sub.site)) continue; // demo + test (lib/testData.js)
    scanned++;
...
// ── E3: owner monthly renewal digest ────────────────────────────────────────
// Once per calendar month per site, a "N renewals to confirm this month" bell +
// Stemfra-brand email to the owner, deep-linked to the CMS Renewals view. Fires
// only when there is at least one venue renewal due in/before the current month;
// idempotent via an existing `membership_renewal_digest` bell stamped with the
// month (so a 12h sweep re-run in the same month is a no-op). The bell always
// posts; the email is gated on the owner_membership notify pref.
async function sweepOwnerRenewalDigests({ dryRun = false, includeDemo = false } = {}) {
  const now = new Date();
  ...
  const { data: subs, error } = await supabase
    .from('site_subscriptions')
    .select('id, site_id, status, current_period_end, amount_cents, cancel_at_period_end, collection_mode, plan:site_products(price_cents, currency), site:sites!inner(status, owner_contact_id, metadata)')
    .eq('collection_mode', 'venue')
    .in('status', ['active', 'renewal_due'])
    .eq('cancel_at_period_end', false)
    .eq('site.status', 'live')
    .not('current_period_end', 'is', null)
    .lte('current_period_end', new Date(monthEndMs).toISOString())
    .limit(5000);
  if (error) { console.error('[membership-digest] load failed:', error.message); return { error: error.message }; }

  // Group due renewals by site.
  const bySite = new Map();
  for (const sub of subs || []) {
    if (!includeDemo && isNonProductionSite(sub.site)) continue;
    if (!bySite.has(sub.site_id)) bySite.set(sub.site_id, { ownerContactId: sub.site.owner_contact_id, subs: [] });
    bySite.get(sub.site_id).subs.push(sub);
  }

  let digestsSent = 0, skipped = 0;
  for (const [siteId, { ownerContactId, subs: due }] of bySite) {
    const dueCount = due.length;
    const overdueCount = due.filter(s => new Date(s.current_period_end).getTime() < nowMs).length;
    const amountCents = due.reduce((sum, s) => sum + (s.amount_cents ?? s.plan?.price_cents ?? 0), 0);
    const currency = due.find(s => s.plan?.currency)?.plan?.currency || 'USD';

    // Idempotency: one digest per site per month.
    const { data: existing } = await supabase
      .from('cms_notifications').select('id')
      .eq('site_id', siteId).eq('type', 'membership_renewal_digest')
      .filter('metadata->>month', 'eq', monthKey).limit(1);
    if (existing && existing.length) { skipped++; continue; }
    if (dryRun) { digestsSent++; continue; }

    // Bell (always).
    await supabase.from('cms_notifications').insert([{
      site_id: siteId, type: 'membership_renewal_digest', category: 'operations',
      title: dueCount === 1 ? '1 membership renewal to confirm' : `${dueCount} membership renewals to confirm`,
      body: `Confirm the payment${dueCount === 1 ? '' : 's'} you collect in person to keep your reports accurate${overdueCount ? ` (${overdueCount} overdue)` : ''}.`,
      href: '/memberships', metadata: { month: monthKey, due: dueCount, overdue: overdueCount },
    }]);

    // Email (gated on the owner_membership pref).
    try {
      const prefs = await getSiteNotifyPrefs(siteId);
      if (prefs.owner_membership && ownerContactId) {
        const { data: owner } = await supabase
          .from('contacts').select('email, auth_user_id').eq('id', ownerContactId).maybeSingle();
        if (owner?.email) {
          const dashboardUrl = await cmsMagicLink(owner.auth_user_id, '/memberships');
          await sendMail({
            fromName: 'Stemfra',
            to: owner.email,
            subject: dueCount === 1 ? '1 membership renewal to confirm' : `${dueCount} membership renewals to confirm`,
            html: emails.ownerRenewalDigest({
              dueCount, overdueCount, monthLabel,
              amountLabel: amountCents ? moneyLabel(amountCents, currency) : null,
              dashboardUrl,
            }),
          });
        }
      }
    } catch (e) { console.error('[membership-digest] owner email failed:', e.message); }
    digestsSent++;
  }
  const summary = { sites: bySite.size, digestsSent, skipped };
  if (!dryRun) console.log('[membership-digest] swept', summary);
  return summary;
}

// Run both passes: E2 (per-sub status + member reminders) then E3 (per-site
// owner digest). The digest self-gates to once per month, so a 12h cadence is
// safe.
async function runMembershipSweeps(opts = {}) {
  await sweepMembershipRenewals(opts);
  await sweepOwnerRenewalDigests(opts);
}
...
function startMembershipRenewalSweeper({ intervalMs = 12 * 3600 * 1000 } = {}) {
  ...
  setTimeout(() => runMembershipSweeps().catch(() => {}), 60000);   // shortly after boot
  const t = setInterval(() => runMembershipSweeps().catch((e) => console.error('[membership-renewal]', e.message)), intervalMs);
  t.unref?.();
  console.log(`✓ Membership renewal sweeper running every ${Math.round(intervalMs / 3600000)}h`);
  return t;
}

module.exports = { startMembershipRenewalSweeper, sweepMembershipRenewals, sweepOwnerRenewalDigests, runMembershipSweeps };
