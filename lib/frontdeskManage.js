...
// Which templates actually ship a member portal (a /account page with
// magic-link sign-in). A site WITHOUT one has no sign-in page at all, so telling
// a visitor to "use the sign-in link on our site" sends them looking for
// something that does not exist. Defaults to "no portal", which is the safe
// answer: we offer to pass it to the team, which always works.
// ⚠ Add a vertical here when its template gains an AccountPage route.
const MEMBER_PORTAL_VERTICALS = new Set(['crossfit', 'massage', 'spa']);
const MEMBER_PORTAL_PATH = '/account';

async function hasMemberPortal(siteId) {
  const { data } = await supabase
    .from('sites').select('vertical:verticals(slug)').eq('id', siteId).maybeSingle();
  return MEMBER_PORTAL_VERTICALS.has(data?.vertical?.slug || '');
}
...
async function runManageTool({ site, member, intent, booking, zone }) {
  ...
  if (!member?.customerId) {
    // Only send them to sign in if this site HAS somewhere to sign in.
    if (await hasMemberPortal(site.id)) {
      return {
        note: `BOOKING NOTE: The visitor wants to ${act} a booking but is NOT signed in, so you cannot verify the booking is theirs and must not change anything. In ONE or TWO short sentences say you can only change a booking once they are signed in, and that a sign-in button is shown. Do NOT write out a URL yourself and do NOT ask for a booking reference as a substitute for signing in.`,
        card: {
          kind: 'handoff_booking',
          title: 'Sign in to change a booking',
          lines: ['We need to know it is you before changing anything.'],
          actions: [
            { label: 'Sign in', href: MEMBER_PORTAL_PATH },
            { label: 'Message the team', value: 'Please pass a message to the team instead.' },
          ],
        },
      };
    }
    return {
      note: `BOOKING NOTE: The visitor wants to ${act} a booking, but this business has NO customer sign-in, so there is no way for you to verify the booking is theirs and you must not change anything. Do NOT mention signing in or a sign-in link — none exists. In ONE or TWO short sentences: say you'll pass it to the team to sort out, and ask for their name and the best email or phone to reply to. Treat it as an escalation.`,
    };
  }
  if (member.suspended) {
  ...
  if (act === 'cancel') {
    if (booking?.confirm !== true) {
      return {
        note: `BOOKING NOTE: A confirmation card for canceling ${label} on ${when} is shown. In ONE short line ask them to confirm. Set booking.confirm=true ONLY when they say yes.`,
        card: {
    ...
    return {
      note: `BOOKING CONFIRMED: Canceled ${label} on ${when}. Confirm warmly in ONE short sentence and mention a confirmation email is on the way. Stop collecting details.`,
      card: { kind: 'booking_done', title: 'Booking canceled', lines: [label, when] },
    };
  }
