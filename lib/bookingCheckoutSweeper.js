...
async function cancelHold(b) {
  if (b.stripe_checkout_session_id) {
    await supabase.from('site_bookings').update({ status: 'cancelled', payment_status: 'failed' })
  ...
  if (b.group_id) {
    ...
    const { count } = await supabase.from('site_bookings')
      ...
      .eq('group_id', b.group_id).neq('status', 'cancelled');
    if (!count) await supabase.from('site_booking_groups').update({ status: 'cancelled' }).eq('id', b.group_id);
  }
}
...
async function sweepOnce() {
  ...
  if (finalized || cancelled) {
    console.log(`[booking-checkout-sweep] finalized ${finalized} paid, cancelled ${cancelled} abandoned`);
  }
}
