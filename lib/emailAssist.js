...
// Tenant-voiced suggested reply for the CMS enquiries inbox (inbox-parity
// arc, 2026-09-01): grounded ONLY on the enquiry + what the owner already
// sent; speaks as the business, not as Stemfra staff.
async function suggestLeadReply({ businessName, leadName, leadMessage, priorReplies }) {
  if (!client) return { error: 'AI assist needs an OpenAI key on the server.' };
  const message = String(leadMessage || '').trim().slice(0, 6000);
  if (!message) return { error: 'This enquiry has no message to reply to.' };
  const prior = (priorReplies || [])
    .map((r, i) => `Reply ${i + 1} already sent: ${String(r).slice(0, 1500)}`)
    .join('\n\n');
  try {
    const r = await client.chat.completions.create({
      model: MODEL,
      messages: [
        {
          role: 'system',
          content: `You write replies on behalf of ${String(businessName || 'a local business').slice(0, 120)}, a local business, `
            + 'to customer enquiries received through its website. Be warm, brief, and concrete: answer what the enquiry asked where possible, '
            + 'and propose one clear next step (booking a visit, a quick call). Never invent prices, availability, dates, or policies; '
            + 'leave a [bracketed placeholder] where a fact is needed. '
            + `Respond as JSON: {"body": "..."} where body follows these rules: ${HTML_RULES}`,
        },
        {
          role: 'user',
          content: [
            `Enquiry from ${String(leadName || 'a visitor').slice(0, 120)}:\n${message}`,
            prior || null,
    ...
    const parsed = JSON.parse(r.choices[0].message.content);
    const body = clean(parsed.body);
    if (!body) return { error: 'The suggestion came back empty. Try again.' };
    return { html: body, model: MODEL };
  } catch (e) {
    console.warn('[email-assist] suggest-lead-reply failed:', e.message);
    return { error: `Suggestion failed: ${e.message}` };
  }
}

module.exports = { refineEmailHtml, draftEmail, suggestLeadReply, REFINE_INSTRUCTIONS };
