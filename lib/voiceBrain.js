...
function buildSystemPrompt({ leadContext, from, transferAvailable, accountContext, oooNote } = {}) {
  return [
...
async function streamReply({ history, leadContext, from, transferAvailable, accountContext, onToken, signal, oooNote}) {
  if (!openai) { const m = "Sorry, I can't take this call right now."; onToken(m); return m; }
  const messages = [{ role: 'system', content: buildSystemPrompt({ leadContext, from, transferAvailable, accountContext, oooNote }) }, ...history];
  let full = '';
  try {
    ...
    const r = await openai.chat.completions.create({
      ...
      messages: [
        { role: 'system', content: [
          ...
          ' "disposition":"qualified"|"not_qualified"|"callback_requested"|"support_request"|"no_interest"|"voicemail",',
          ...
          '"disposition": support calls are always "support_request". Sales: "qualified" when they run a fitting business AND showed real interest, "callback_requested" when they mainly asked to be called back, "no_interest" when they declined, else "not_qualified". If the other side was an answering machine, voicemail greeting, or phone menu (phrases like "leave a message", "at the tone", "press 1"), the disposition is ALWAYS "voicemail" — a voicemail is never "qualified" and its sentiment is "neutral".',
