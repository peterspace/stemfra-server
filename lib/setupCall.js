...
async function publicConfig() {
  const away = await ooo.currentOrNext();
  return {
    ...
    // Out of office (team away): the booking page shows this; slots inside are hidden.
    outOfOffice: away ? { from: away.from, to: away.to, note: away.note || null, label: ooo.describe(away) } : null,
  };
}
...
async function getDaySlots(dateStr) {
  ...
  if (await ooo.isOutOfOffice(day.toFormat('yyyy-MM-dd'))) return []; // team away (crm_settings.out_of_office)
