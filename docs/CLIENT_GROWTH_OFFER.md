...
| **Seasonal holiday messages** | ✅ SHIPPED 2026-08-27 | `lib/seasonalGreetings.js`: New Year / July 4 / Thanksgiving (4th Thu) / Christmas; per-site toggle + OWNER-EDITABLE message (CMS Notifications → Automated emails → Seasonal greetings); once per customer per holiday per year, 09:00–20:00 site-local, hourly sweep; pure care message (no CTA). Calendar unit-tested; live render test sent |
| **Review timing: on completion** | ✅ SHIPPED 2026-08-27 | Peter's call: the ask lands MINUTES after the owner marks a visit completed (5-min fast lane), so staff can say "the review link is on its way"; the ~2-day sweep remains the fallback for never-marked bookings |
| SMS variants of lifecycle emails | 🟡 NEXT (pending Twilio campaign decision) | Review link + birthday etc. by SMS to `sms_opt_in` customers. Plan: ONE new A2P campaign under the existing Stemfra brand + 1–2 DEDICATED numbers for tenant-customer traffic (shared across all tenants, "ShopName:" sender prefix, STOP → per-customer suppression). NO per-tenant numbers/brands until ~25+ actively-texting tenants (ISV model then). Keep tenant traffic OFF Mark's/CRM numbers (campaign-description match, STOP scoping, reputation isolation) |
